...
interface IProps {
	record: RestaurantCardOutput<RatingDetailOutput | ReservationDetailOutput>;
...
export const RestaurantCard: React.FC<IProps> = (props) => {
	const { record, navigateToRestaurantView, showStatus, showCity, isActive, onEditRecord } = props;
	...
	return (
		<SuperLink
			to={navigateToRestaurantView ? `/restaurant/${record.name}/home` : ''}
							...
							<VStack align="stretch">
								{(detail as ReservationDetailOutput).date_of_reservation && (
									<React.Fragment>
										<IconLabel
											icon={FiCalendar}
											label={formatter.format(new Date((detail as ReservationDetailOutput).date_of_reservation ?? ''))}
										/>
										<IconLabel
											icon={FiClock}
											label={new Date((detail as ReservationDetailOutput).date_of_reservation ?? '').toLocaleTimeString(
												[],
												{
													hour: '2-digit',
													minute: '2-digit',
